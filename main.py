...
with st.form("my-form"):
    row = st.slider("ピコ[テンポ遅め]← →[テンポ早め]", -2, 2, 0)
    column = st.slider("パコ[複雑、内省的]← →[単純、愉快]", -2, 2, 0)

    submitted = st.form_submit_button("ピ！")
if submitted: 
    st.audio(mp3_files[(df.iloc[row, column])], format="audio/mpeg", autoplay=True)

st.audio("music/01 You Know What I Mean.mp3", format="audio/mpeg", autoplay=True)
st.audio(mp3_files[df.iloc[2, 0]], format="audio/mpeg", autoplay=True)
